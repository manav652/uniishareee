    ...
    <AppShell>
      <div className="container max-w-7xl py-8 md:py-12 space-y-16">
            ...
            <div className="space-y-6 animate-fade-up">
              ...
              <div className="flex flex-wrap gap-3">
                <Button asChild size="lg" className="bg-gradient-primary shadow-glow text-base">
                  <Link to="/browse">Explore now <ArrowRight className="ml-1 h-4 w-4" /></Link>
                </Button>
                <Button asChild size="lg" variant="outline" className="text-base border-primary/30 hover:bg-primary/10">
                  <Link to="/upload"><Upload className="mr-1 h-4 w-4" /> List a project</Link>
                </Button>
              </div>
            </div>
            <div className="relative hidden md:block">
              <div className="absolute inset-0 bg-primary/30 blur-3xl rounded-full" />
              <img src={heroImg} alt="Student collaborating on academic projects" width={1024} height={768}
        ...
        <section className="grid md:grid-cols-3 gap-5">
          {[
            { icon: ShieldCheck, title: "University verified", desc: "Only students with a valid academic email can sign up." },
            { icon: MessageSquare, title: "Real-time chat", desc: "Negotiate, ask questions and share details instantly." },
            { icon: Sparkles, title: "Curated content", desc: "Final-year projects, code, notes and study material in one place." },
          ].map((f) => (
